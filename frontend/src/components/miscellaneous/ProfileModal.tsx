 ...
 *
 * State management:
 * - Modal open/close: useDisclosure (local Chakra UI state)
...
const ProfileModal: React.FC<ProfileModalProps> = ({ user, children }) => {
  ...
  const handlePicUpload = (file: File | undefined): void => {
    ...
    fetch("https://api.cloudinary.com/v1_1/difmt49ax/image/upload", { method: "post", body: formData })
      ...
      .catch(() =>
        toast({ title: "Upload failed", status: "error", duration: 4000, isClosable: true, position: "top" })
      )
    ...
    <>
      {children
        ...
        : (
          <Tooltip label="View Profile" hasArrow placement="bottom">
            <IconButton aria-label="View profile" icon={<ViewIcon />}
              onClick={onOpen} variant="nav" size="sm" />
          </Tooltip>
        )
      }
      ...
      <Modal size="sm" onClose={handleClose} isOpen={isOpen} isCentered>
        ...
        <ModalContent
          ...
          borderRadius="16px" overflow="hidden" mx={4}
        >
          <Box h="80px" bgGradient="linear(to-r, #833AB4, #E1306C, #F77737)" />
          ...
          <ModalBody pb={6} pt={0}>
            <VStack spacing={4} align="center">
              ...
              <Box position="relative" mt="-40px">
                <Image
                  borderRadius="full" boxSize="80px"
                  src={previewPic} alt={user.name}
                  border="3px solid" borderColor="bg-surface"
                  objectFit="cover" bg="bg-elevated"
                ...
                {isOwnProfile && (
                  <>
                    ...
                    <Tooltip label="Change photo" hasArrow>
                      <IconButton
                        aria-label="Change photo" icon={<EditIcon />}
              ...
              {isEditing ? (
                <Input
                  ...
                  textAlign="center" fontWeight="bold" fontSize="lg"
                  color="text-primary" bg="bg-input"
                  border="1px solid" borderColor="accent" borderRadius="8px"
              ...
              {isOwnProfile && (
                isEditing ? (
                  ...
                  <Button
                    ...
                    leftIcon={<EditIcon />} color="text-primary"
